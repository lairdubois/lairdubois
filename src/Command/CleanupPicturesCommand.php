...
class CleanupPicturesCommand extends AbstractContainerAwareCommand {
...
EOT
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$queryBuilder
			->select(array( 'c', 'ps' ))
			->from('App\Entity\Core\Comment', 'c')
		...
		$queryBuilder
			->select(array( 'm', 'ps' ))
			->from('App\Entity\Message\Message', 'm')
		...
		foreach ($finds as $find) {
			...
			if ($content instanceof \App\Entity\Find\Content\Gallery) {
		...
		$queryBuilder
			->select(array( 'q', 'mp' ))
			->from('App\Entity\Faq\Question', 'q')
			->leftJoin('q.mainPicture', 'mp')
		;
		...
		$queryBuilder
			...
			->from('App\Entity\Knowledge\Software', 's')
			->leftJoin('s.mainPicture', 'mp')
		...
		$queryBuilder
			->select(array( 'v', 'd' ))
			->from('App\Entity\Knowledge\Value\Picture', 'v')
			->leftJoin('v.data', 'd')
		;
		...
		$queryBuilder
			->select(array( 'g', 'ps' ))
			->from('App\Entity\Core\Block\Gallery', 'g')
		...
		$queryBuilder
			->select(array( 'm' ))
			->from('App\Entity\Knowledge\Wood\Texture', 'm')
		;
		...
		$queryBuilder
			->select(array( 't', 'mp' ))
			->from('App\Entity\Youtook\Took', 't')
		...
		$queryBuilder
			->select(array( 'c', 'mp' ))
			->from('App\Entity\Collection\Collection', 'c')
