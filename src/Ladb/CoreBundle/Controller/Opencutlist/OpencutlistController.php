...
class OpencutlistController extends AbstractController {
    ...
    const BRANCH_DEV = '7.1.0';
    const BRANCH_DEV2 = '8.0.0';
	...
	public function manifestAction(Request $request, $env = 'prod') {
        ...
        if ($access->getIsEnvDev()) { // && $access->getClientOclVersion() == self::BRANCH_DEV.'-dev') { //} && in_array($access->getClientSketchupLocale(), self::DEV_LANGUAGES)) {
            if ($access->getClientOclVersion() == self::BRANCH_DEV2.'-dev') {
                $branch = self::BRANCH_DEV2;
            } else {
...
//            return $this->redirect('http://opencutlist.lairdubois.fr/ladb_opencutlist.rbz');
	...
	public function tutorialsAction(Request $request, $env = 'prod') {
		...
		return $this->redirect('https://raw.githubusercontent.com/lairdubois/lairdubois-opencutlist-sketchup-extension/'.$branch.'/docs/json/tutorials.json');
	}
	...
	public function docsAction(Request $request, $env = 'prod') {
		...
		switch ($request->get('page', '')) {
            ...
            case 'cutlist.packing.write':
                $path = '/features/parts/parts-list/packing/export';
                break;
    ...
    public function changelogAction(Request $request, $env = 'prod') {
        ...
        return $this->redirect('https://raw.githubusercontent.com/lairdubois/lairdubois-opencutlist-sketchup-extension/'.$branch.'/CHANGELOG.md');
    }
