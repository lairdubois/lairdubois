...
class CreationController extends Controller {
	 ...
	 * @Method("POST")
	 * @Template("LadbCoreBundle:Creation:new.html.twig")
	 */
	public function createAction(Request $request) {
		...
		if ($form->isValid()) {
			...
			$this->getUser()->incrementDraftCreationCount();
	 ...
	 * @Method("POST")
	 * @Template("LadbCoreBundle:Creation:edit.html.twig")
	 */
	...
	public function plansAction(Request $request, $id, $filter = "recent", $page = 0) {
		...
		$pageUrls = $paginatorUtils->generatePrevAndNextPageUrl('core_creation_plans_filter_page', array( 'filter' => $filter ), $page, $paginator->count());
	...
	public function howtosAction(Request $request, $id, $filter = "recent", $page = 0) {
		...
		$pageUrls = $paginatorUtils->generatePrevAndNextPageUrl('core_creation_howtos_filter_page', array( 'filter' => $filter ), $page, $paginator->count());
	...
	public function inspirationsAction(Request $request, $id, $filter = "recent", $page = 0) {
		...
		$pageUrls = $paginatorUtils->generatePrevAndNextPageUrl('core_creation_inspirations_filter_page', array( 'filter' => $filter ), $page, $paginator->count());
	...
	public function reboundsAction(Request $request, $id, $filter = "recent", $page = 0) {
		...
		$pageUrls = $paginatorUtils->generatePrevAndNextPageUrl('core_creation_rebounds_filter_page', array( 'filter' => $filter ), $page, $paginator->count());
	...
	/**
	 * @Route("/{id}/sticker.png", requirements={"id" = "\d+"}, name="core_creation_sticker")
	 */
	...
	public function listAction(Request $request, $page = 0) {
		...
		$searchParameters = $searchUtils->searchPaginedEntities(
			...
			'fos_elastica.index.ladb.creation',
		...
		if ($request->isXmlHttpRequest()) {
			if ($layout == 'choice') {
				return $this->render('LadbCoreBundle:Creation:list-choice-xhr.html.twig', $parameters);
			} else {
				return $this->render('LadbCoreBundle:Creation:list-xhr.html.twig', $parameters);
			}
		}
		...
		if ($layout == 'choice') {
			return $this->render('LadbCoreBundle:Creation:list-choice.html.twig', $parameters);
		}
	...
	public function showAction(Request $request, $id) {
		...
		$userCreations = $explorableUtils->getPreviousAndNextPublishedUserExplorables($creation, $creationRepository, $creation->getUser()->getPublishedCreationCount());
		$similarCreations = $explorableUtils->getSimilarExplorables($creation, 'fos_elastica.index.ladb.creation', Creation::CLASS_NAME, $userCreations);
		...
		$searchUtils = $this->get(SearchUtils::NAME);
		$searchableWoodCount = $searchUtils->searchEntitiesCount(array( new \Elastica\Query\Match('name', $woodsString) ), null, 'fos_elastica.index.ladb.wood');
