...
class NotificationRepository extends AbstractEntityRepository {
	...
	private function _applyCommonFilter(&$queryBuilder, $filter) {
		if ('activity-like' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Like')
			;
		} elseif ('activity-comment' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Comment')
			;
		} elseif ('activity-follow' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Follow')
			;
		} elseif ('activity-publish' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Publish')
			;
		} elseif ('activity-vote' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Vote')
			;
		} elseif ('activity-join' == $filter) {
			$queryBuilder
				->andWhere('a INSTANCE OF \\Ladb\\CoreBundle\\Entity\\Activity\\Join')
			;
		}
		$queryBuilder
